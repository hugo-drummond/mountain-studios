// Preview runner — executes redesigned templates with mock data and writes HTML files
import { writeFileSync, readFileSync } from 'fs'

// ─── Inline all shared types and helpers ───────────────────────────────────
...
interface StockImages { hero: string; about: string; cards: string[]; avatar: string }
interface GeneratedContent {
  heroEyebrow: string; heroAccent?: string; tagline: string; heroSubtitle: string
  ctaPrimary: string; ctaSecondary: string; ctaNote?: string; badge?: string
  servicesHeading: string
  services: { name: string; description: string; tags: string[]; icon?: string; serviceImageQuery?: string }[]
  galleryHeading: string; aboutHeading: string; aboutText: string; aboutMission?: string
  stats: { value: string; label: string; sublabel?: string }[]
  contactHeading: string; contactHours?: string
  processSteps?: { step: string; title: string; description: string }[]
  stepsHeading?: string; projectCaptions?: string[]
  testimonial?: { quote: string; author: string; rating: number }
  testimonials?: { quote: string; author: string; rating: number }[]
  imageMood?: string; heroImageQuery?: string; aboutImageQuery?: string; galleryImageQueries?: string[]
  features?: { name: string; description: string; imageQuery?: string }[]
}
interface TemplateData {
  content: GeneratedContent; businessName: string; businessCategory: BusinessCategory
  primaryColor: string; secondaryColor: string; pages: string[]
  images: string[]; stockImages: StockImages; variant: TemplateVariant
  locationInfo: ReturnType<typeof getLocationInfo>
}

function getLocationInfo(country: string) {
...
function buildCssVars(fonts: { headingFamily: string }, primaryColor: string, secondaryColor: string, theme: Theme = 'dark'): string {
  ...
  const vars = theme === 'light' ? `
      --bg: #fafafa; --bg-alt: #f0f0f0; --card-bg: #ffffff;
      --text: #1a1a1a; --text-muted: #555555; --border: rgba(0,0,0,0.08);` : `
      --bg: #0f0f0f; --bg-alt: #141414; --card-bg: #1a1a1a;
      --text: #f5f5f0; --text-muted: #a3a3a0; --border: rgba(255,255,255,0.08);`
  return `
    :root {
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #ms-mob-menu { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(16px); z-index: 150; cursor: pointer; align-items: center; justify-content: center; }
    #ms-mob-menu a { color: #fff; text-decoration: none; font-family: var(--heading-font); font-size: 1.6rem; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; padding: 0.25rem 0; }
...
function buildMobileMenu(content: GeneratedContent, links?: { label: string; href: string }[]): string {
  const menuLinks = links || [{ label: 'Services', href: '#services' }, { label: 'About', href: '#about' }, { label: 'Contact', href: '#contact' }]
  return `
...
function buildImagePool(images: string[], stockImages: { hero: string; cards: string[] }, businessName: string): string[] {
  ...
  while (pool.length < 20) pool.push(`https://picsum.photos/seed/${encodeURIComponent(businessName)}-img${pool.length}/600/400`)
  return pool
}

const categoryTestimonials: Record<string, { quote: string; author: string; rating: number }[]> = {
  'food-hospitality': [{ quote: 'The food was incredible and the service even better.', author: 'Sarah M., Regular Guest', rating: 5 }, { quote: 'Celebrated our anniversary here — perfect atmosphere.', author: 'David K., Cape Town', rating: 5 }],
  'retail': [{ quote: 'Always find exactly what I\'m looking for.', author: 'Priya N., Loyal Customer', rating: 5 }, { quote: 'Great selection and fair prices. My go-to shop.', author: 'James V.', rating: 5 }],
  'trades-construction': [{ quote: 'Arrived on time, clean job, exact quote. Rare.', author: 'Mark D., Homeowner', rating: 5 }, { quote: 'Excellent workmanship and no mess left behind.', author: 'Zanele K.', rating: 5 }],
  'health-wellness': [{ quote: 'Caring, thorough, and always takes time to explain.', author: 'Rachel S., Regular Client', rating: 5 }, { quote: 'Finally found a practice that actually listens.', author: 'Thomas B.', rating: 5 }],
  'professional': [{ quote: 'Handled our case with real expertise and clear communication.', author: 'Linda P., Business Client', rating: 5 }, { quote: 'Responsive, knowledgeable, genuinely invested.', author: 'Andrew F., Director', rating: 5 }],
  'fitness-sport': [{ quote: 'Best shape of my life. Trainers push you just right.', author: 'Chris W., Member', rating: 5 }, { quote: 'Great community vibe and top-notch facilities.', author: 'Naledi M.', rating: 5 }],
  'pets': [{ quote: 'They treat our dogs like family. Wouldn\'t trust anyone else.', author: 'Emma H.', rating: 5 }, { quote: 'Gentle with nervous animals and always go extra mile.', author: 'Fatima A.', rating: 5 }],
  'home-services': [{ quote: 'Reliable and our house has never looked better.', author: 'Karen L., Homeowner', rating: 5 }, { quote: 'On time, brilliant job. Will use again.', author: 'Sipho M.', rating: 5 }],
  'education': [{ quote: 'My child\'s confidence has grown so much. Teachers wonderful.', author: 'Nicole V., Parent', rating: 5 }, { quote: 'Patient and makes learning enjoyable.', author: 'Brian T.', rating: 5 }],
  'events-entertainment': [{ quote: 'Made our event absolutely unforgettable.', author: 'Tamara J., Bride', rating: 5 }, { quote: 'Professional from start to finish.', author: 'Kevin R.', rating: 5 }],
  'automotive': [{ quote: 'Honest pricing and explains exactly what\'s needed.', author: 'Johan P.', rating: 5 }, { quote: 'Quick turnaround and quality work.', author: 'Michelle D.', rating: 5 }],
  'creative': [{ quote: 'Captured exactly what we envisioned — real creative talent.', author: 'Samantha L.', rating: 5 }, { quote: 'On time, on brief, exceptional quality.', author: 'Ruan V.', rating: 5 }],
  'property': [{ quote: 'Found us the perfect home in two weeks. Zero pressure.', author: 'The Naidoo Family', rating: 5 }, { quote: 'Professional, transparent, had our best interests at heart.', author: 'Carol M., Seller', rating: 5 }],
  'tech-digital': [{ quote: 'Traffic is up 300% since launch. Transformed our presence.', author: 'Laura H., Founder', rating: 5 }, { quote: 'They understood our business, not just the tech.', author: 'Peter G., MD', rating: 5 }],
  'other': [{ quote: 'Exceeded our expectations in every way.', author: 'Amanda S.', rating: 5 }, { quote: 'Professional and a pleasure to work with.', author: 'Daniel K.', rating: 5 }],
}

function getFallbackTestimonials(content: GeneratedContent, category: string) {
  if (content.testimonials && content.testimonials.length >= 3) return content.testimonials.slice(1, 3)
  return categoryTestimonials[category] || categoryTestimonials['other']!
}

function resolveNavLinks(pages: string[]) {
  const lp = pages.map(p => p.toLowerCase())
  const links: { label: string; href: string }[] = []
  if (lp.some(p => ['services','menu','treatments','classes','courses','products'].some(k => p.includes(k)))) links.push({ label: 'Services', href: '#services' })
  if (lp.some(p => ['gallery','portfolio','work','projects'].some(k => p.includes(k)))) links.push({ label: 'Gallery', href: '#gallery' })
  if (lp.some(p => p.includes('about'))) links.push({ label: 'About', href: '#about' })
  if (lp.some(p => ['contact','booking','appointment'].some(k => p.includes(k)))) links.push({ label: 'Contact', href: '#contact' })
  return { allLinks: links, navServices: links.some(l => l.href==='#services'), navGallery: links.some(l => l.href==='#gallery'), navAbout: links.some(l => l.href==='#about'), navContact: links.some(l => l.href==='#contact') }
}
...
function buildBurgerButton(color: string = '#fff'): string {
  return `<button class="ms-burger" aria-label="Menu"><span style="background:${color}"></span><span style="background:${color}"></span><span style="background:${color}"></span></button>`
}

function buildFooter(businessName: string, content: GeneratedContent, theme: Theme = 'dark'): string {
  const serviceLinks = content.services.slice(0,4).map(s => `<a href="#services" style="color:var(--text-muted);text-decoration:none;font-size:0.85rem;display:block;margin-bottom:0.6rem">${s.tags?.[0] || s.name}</a>`).join('\n')
  return `
  <footer style="padding:4rem 2rem 2rem;background:${theme==='light'?'#f0f0f0':'#0a0a0a'};border-top:1px solid var(--border)">
    <div class="ms-grid" style="max-width:1100px;margin:0 auto;display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:3rem">
      ...
      <div><div style="font-family:var(--body-font);font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text);margin-bottom:1rem;font-weight:600">Services</div>${serviceLinks}</div>
      <div><div style="font-family:var(--body-font);font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text);margin-bottom:1rem;font-weight:600">Company</div>
        <a href="#about" style="color:var(--text-muted);text-decoration:none;font-size:0.85rem;display:block;margin-bottom:0.6rem">About</a>
      ...
      <div><div style="font-family:var(--body-font);font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text);margin-bottom:1rem;font-weight:600">Legal</div>
        <a href="#" style="color:var(--text-muted);text-decoration:none;font-size:0.85rem;display:block;margin-bottom:0.6rem">Privacy</a>
        <a href="#" style="color:var(--text-muted);text-decoration:none;font-size:0.85rem;display:block;margin-bottom:0.6rem">Terms</a>
      </div>
    </div>
    <div style="max-width:1100px;margin:2rem auto 0;padding-top:2rem;border-top:1px solid var(--border);text-align:center">
      <div style="font-family:var(--body-font);font-size:0.8rem;color:var(--text-muted)">&copy; 2024 ${businessName}. All rights reserved.</div>
    </div>
  </footer>`
}
    ...
    <section id="about" style="padding:100px 0;background:var(--bg-alt)">
      <div class="ms-grid" style="max-width:1100px;margin:0 auto;padding:0 2rem;display:grid;grid-template-columns:1fr 1fr;gap:4rem;align-items:start">
        <div>
          ...
          ${content.aboutMission ? `<p style="font-family:var(--body-font);font-size:1.1rem;color:var(--text);font-style:italic;font-weight:600;margin:0 0 1.5rem 0">${content.aboutMission}</p>` : ''}
          ${paragraphs.map(p => `<p style="font-family:var(--body-font);font-size:1rem;color:var(--text-muted);line-height:1.8;margin:0 0 1rem 0">${p}</p>`).join('')}
        </div>
        <div>
          <div class="ms-grid" style="display:grid;grid-template-columns:repeat(2,1fr);gap:1.5rem;padding-top:1rem">
            ${content.stats.slice(0,4).map(s => `<div>
  ...
  return `
    <section style="padding:80px 0;background:var(--bg)">
      <div style="max-width:700px;margin:0 auto;padding:0 2rem;text-align:center">
        ...
        <div style="color:#f59e0b;font-size:1rem;letter-spacing:0.1em">${'&#9733;'.repeat(content.testimonial.rating)}</div>
      </div>
    </section>`
}
    ...
    <section id="contact" style="padding:100px 0;background:var(--bg)">
      <div style="max-width:1100px;margin:0 auto;padding:0 2rem">
        ...
        <div class="ms-grid" style="display:grid;grid-template-columns:1fr 1.2fr;gap:4rem;align-items:start">
          <div>
            <div style="margin-bottom:1.5rem"><div style="font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--primary);margin-bottom:0.5rem">Address</div>
            <p style="color:var(--text-muted)">${locationInfo.address}, ${locationInfo.city}</p></div>
            <div style="margin-bottom:1.5rem"><div style="font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--primary);margin-bottom:0.5rem">Phone</div>
            <p style="color:var(--text-muted)">${locationInfo.phone}</p></div>
            ${content.contactHours ? `<div><div style="font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--primary);margin-bottom:0.5rem">Hours</div><p style="color:var(--text-muted)">${content.contactHours}</p></div>` : ''}
          </div>
          <form style="display:flex;flex-direction:column;gap:1.25rem" onsubmit="return false">
            ...
            <textarea placeholder="Message" rows="4" style="font-family:var(--body-font);padding:0.9rem 1.25rem;background:var(--card-bg);border:1px solid var(--border);border-radius:12px;color:var(--text);font-size:0.95rem;outline:none;resize:none"></textarea>
            <button type="submit" style="font-family:var(--body-font);padding:1rem 2.5rem;background:var(--primary);color:#fff;border:none;border-radius:12px;font-size:1rem;font-weight:600;cursor:pointer">Send Message</button>
          </form>
        </div>
      </div>
    </section>`
...
function buildHead(businessName: string, fonts: { heading: string; headingFamily: string }, primaryColor: string, secondaryColor: string, theme: Theme = 'dark'): string {
  const fontUrl = `https://fonts.googleapis.com/css2?family=${fonts.heading}&family=Inter:wght@300;400;500;600&display=swap`
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
// ─── Now load and eval the redesigned template files ───────────────────────

const TEMPLATES_DIR = '/Users/hugodrummond/agency-backend/templates'

// Read all redesigned files
const redesignedCode = [
  'redesigned-base-variants.ts',
  'redesigned-food-retail-pets.ts',
  'redesigned-food-hospitality.ts',
  'redesigned-health-fitness-education.ts',
  'redesigned-health-wellness.ts',
  'redesigned-professional-tech.ts',
  'redesigned-trades-home-auto.ts',
  'redesigned-property-creative-events.ts',
].map(f => readFileSync(`${TEMPLATES_DIR}/${f}`, 'utf8')).join('\n\n')

// Write combined file for tsx to process
const combinedPath = '/tmp/all-templates-combined.ts'
writeFileSync(combinedPath, `
// Stub out Next.js imports
const NextRequest = {} as any
const NextResponse = { json: (d: any) => d } as any

// All shared helpers are defined above in this file
${redesignedCode}

export {
  buildVisualTemplate, buildServiceTemplate, buildPortfolioTemplate,
  buildFoodHospitalityTemplate, buildRetailTemplate, buildPetsTemplate,
  buildHealthWellnessTemplate, buildFitnessTemplate, buildEducationTemplate,
  buildProfessionalTemplate, buildTechDigitalTemplate,
  buildTradesTemplate, buildHomeServicesTemplate, buildAutomotiveTemplate,
  buildPropertyTemplate, buildCreativeTemplate, buildEventsTemplate,
}
`)

console.log('Combined template file written')


// ─── UTILITY STUBS ───
const SERVICE_ICONS = ['🔨','⚙️','🏗️','🪚','🔧','🛠️','🏠','🌿','🚗','💡','📐','🔑','🧱','🪟','🪣']
function mapIcon(icon: string | undefined, idx: number): string { return icon || SERVICE_ICONS[idx % SERVICE_ICONS.length] || '✦' }
function evenGrid(items: unknown[], cols: number) {
...
function buildFoodHospitalityTemplate(data: TemplateData): string {
  ...
  <style>
    ...
    @media(max-width:768px){ #food-nav { padding:1rem 1.25rem; background:${inkDark} !important; } #food-nav > div > a:not(.food-nav-cta) { display:none!important; } #food-nav label { display:block!important; } .food-nav-cta { display:none!important; } }
  </style>
  <script>
...
function buildRetailTemplate(data: TemplateData): string {
  ...
  const heroSection = `
  <section style="position:relative;min-height:100vh;box-sizing:border-box;padding-bottom:68px;display:grid;grid-template-columns:1fr 1fr;background:${bg}">
...
<section class="hw-hero-section" style="min-height:100vh;box-sizing:border-box;display:grid;grid-template-columns:1fr 1fr;background:${hw.bg};position:relative;padding-bottom:48px">
...
<section class="edu-hero" style="background:${edu.bgWarm};min-height:92vh;display:flex;align-items:center;overflow:hidden;position:relative">
...
${buildStandardNav(businessName, content, navFlags)}
  ...
  <section style="position:relative;min-height:100vh;box-sizing:border-box;padding-bottom:70px;display:flex;align-items:center;overflow:hidden;background:${bg};z-index:1">
...
function buildCreativeTemplate(data: TemplateData): string {
  ...
  const heroSection = `
  <section style="background:${ink};min-height:100vh;box-sizing:border-box;padding-bottom:58px;display:grid;grid-template-columns:1fr 1fr;overflow:hidden;position:relative" class="ms-grid">
...
interface MockDef { name: string; cat: BusinessCategory; color: string; eyebrow: string; tagline: string; sub: string; cta: string; img: string }

const mockDataByCategory: Record<string, MockDef> = {
  visual:          { name: 'Harvest Kitchen',   cat: 'food-hospitality',    color: '#c9783a', eyebrow: 'CAPE TOWN DINING',      tagline: 'Where every plate<br/><em>tells a story</em>',       sub: 'Seasonal menus crafted from local ingredients.', cta: 'Reserve a Table',     img: '1414235077428-338989a2e8c0' },
  service:         { name: 'Vantage Advisory',  cat: 'professional',        color: '#2d5be3', eyebrow: 'PROFESSIONAL SERVICES', tagline: 'Clarity in<br/><em>every decision</em>',              sub: 'Strategic advisory for businesses navigating complex challenges.', cta: 'Book a Consultation', img: '1497366216548-37526070297c' },
  portfolio:       { name: 'Lumière Studio',    cat: 'creative',            color: '#c9a96e', eyebrow: 'VISUAL STORYTELLING',  tagline: 'We craft images that<br/><em>move people</em>',         sub: 'Editorial photography & visual direction for brands.', cta: 'View Our Work',       img: '1506905925346-21bda4d32df4' },
  foodHospitality: { name: 'Harvest Kitchen',   cat: 'food-hospitality',    color: '#c9783a', eyebrow: 'CAPE TOWN DINING',     tagline: 'Where every plate<br/><em>tells a story</em>',          sub: 'Seasonal menus, local ingredients. Reserve your table.', cta: 'Reserve a Table',     img: '1414235077428-338989a2e8c0' },
  retail:          { name: 'Maison Veld',        cat: 'retail',              color: '#2d1a4a', eyebrow: 'CAPE TOWN BOUTIQUE',  tagline: 'Curated style,<br/><em>every season</em>',              sub: 'Handpicked fashion and lifestyle pieces for quality-lovers.', cta: 'Shop Now',            img: '1441984904996-e0b6ba687e04' },
  pets:            { name: 'Paws & Co.',          cat: 'pets',               color: '#e87c3e', eyebrow: 'PET CARE SPECIALISTS', tagline: 'Your pet deserves<br/><em>the very best</em>',          sub: 'Grooming, boarding, and veterinary care with love.', cta: 'Book an Appointment', img: '1450778869180-41d0601e046e' },
  healthWellness:  { name: 'Serenity Health',    cat: 'health-wellness',     color: '#4a9b8e', eyebrow: 'HOLISTIC WELLNESS',   tagline: 'Your path to<br/><em>lasting wellness</em>',            sub: 'Integrative health solutions combining clinical expertise and compassionate care.', cta: 'Book a Consultation', img: '1571019613454-1cb2f99b2d8b' },
  fitness:         { name: 'Apex Athletics',     cat: 'fitness-sport',       color: '#e63946', eyebrow: 'ELITE PERFORMANCE',   tagline: 'Train harder.<br/><em>Perform better.</em>',            sub: 'State-of-the-art facilities and expert coaching.', cta: 'Start Training',     img: '1534438327276-14e5300c3a48' },
  education:       { name: 'Bright Minds',        cat: 'education',          color: '#2563eb', eyebrow: 'LEARNING CENTRE',     tagline: 'Knowledge that<br/><em>transforms lives</em>',          sub: 'Personalized tutoring and enrichment for every learning style.', cta: 'Enrol Now',           img: '1503676260728-1c00da094a0b' },
  professional:    { name: 'Vantage Advisory',   cat: 'professional',        color: '#1e3a5f', eyebrow: 'PROFESSIONAL SERVICES',tagline: 'Clarity in<br/><em>every decision</em>',              sub: 'Strategic advisory for businesses and ambitious growth.', cta: 'Book a Consultation', img: '1497366216548-37526070297c' },
  techDigital:     { name: 'Nexus Digital',       cat: 'tech-digital',       color: '#00a3cc', eyebrow: 'DIGITAL AGENCY',      tagline: 'We build what<br/><em>others dream</em>',               sub: 'Full-stack digital solutions: web, mobile, and AI-powered products.', cta: 'Start a Project',    img: '1518770660439-4636190af475' },
  trades:          { name: 'Apex Builders',       cat: 'trades-construction', color: '#e67e22', eyebrow: 'MASTER CRAFTSMEN',  tagline: 'Built to last.<br/><em>Built to impress.</em>',           sub: 'Quality construction and renovation delivered on time.', cta: 'Get a Free Quote',   img: '1504307651254-35680f356dfd' },
  homeServices:    { name: 'Homefront Co.',        cat: 'home-services',      color: '#27ae60', eyebrow: 'HOME SPECIALISTS',   tagline: 'Your home,<br/><em>our expertise</em>',                 sub: 'Professional cleaning, maintenance, and improvement for every home.', cta: 'Get a Quote',         img: '1556909114-f6e7ad7d3136' },
  automotive:      { name: 'Precision Auto',       cat: 'automotive',         color: '#c0392b', eyebrow: 'AUTO SPECIALISTS',   tagline: 'Performance meets<br/><em>precision</em>',               sub: 'Expert mechanical services for all makes and models.', cta: 'Book a Service',     img: '1492144534655-ae79c964c9d7' },
  property:        { name: 'Meridian Realty',      cat: 'property',           color: '#8b6914', eyebrow: 'LUXURY REAL ESTATE', tagline: 'Find your<br/><em>perfect home</em>',                   sub: "Exceptional properties in Cape Town's most sought-after locations.", cta: 'View Properties',    img: '1600585154340-be6161a56a0c' },
  creative:        { name: 'Form & Function',      cat: 'creative',           color: '#9b59b6', eyebrow: 'CREATIVE STUDIO',    tagline: 'Design that<br/><em>defines brands</em>',               sub: 'Brand identity, digital design, and creative direction.', cta: 'See Our Work',        img: '1558618666-fcd25c85cd64' },
  events:          { name: 'Céleste Events',       cat: 'events-entertainment',color: '#d4a017', eyebrow: 'EVENT SPECIALISTS', tagline: 'Moments made<br/><em>unforgettable</em>',               sub: 'Bespoke event planning and production for every occasion.', cta: 'Plan Your Event',    img: '1492684223066-81342ee5ff30' },
}

const categoryServices: Record<string, typeof PLACEHOLDER_SERVICES> = {
  'food-hospitality': [
    { name: 'Dine In',          description: 'Seasonal menus crafted from local produce, served in a warm and welcoming space.', tags: ['À la Carte', 'Seasonal'], icon: '✦', serviceImageQuery: '' },
    { name: 'Private Events',   description: 'Intimate dinners, corporate functions, and celebrations catered with care.',        tags: ['Bespoke', 'Full Catering'], icon: '◈', serviceImageQuery: '' },
    { name: 'Takeaway & Deli',  description: 'Freshly prepared dishes and pantry staples to enjoy at home.',                      tags: ['Fresh Daily', 'Local'],    icon: '◉', serviceImageQuery: '' },
    { name: 'Chef\'s Table',    description: 'An exclusive six-course experience curated personally by our head chef.',           tags: ['Premium', 'Limited Seats'], icon: '◆', serviceImageQuery: '' },
  ],
  'retail': [
    { name: 'In-Store Shopping',    description: 'Browse our curated collections across fashion, lifestyle, and homeware.',        tags: ['Curated', 'In-Season'],   icon: '✦', serviceImageQuery: '' },
    { name: 'Personal Styling',     description: 'One-on-one sessions with our in-house stylist to build your signature look.',    tags: ['By Appointment', 'Bespoke'], icon: '◈', serviceImageQuery: '' },
    { name: 'Gift Wrapping',        description: 'Complimentary gift wrapping and personalised notes on every purchase.',         tags: ['Complimentary', 'Same Day'], icon: '◉', serviceImageQuery: '' },
    { name: 'Online Ordering',      description: 'Shop our full range online with next-day delivery across Cape Town.',           tags: ['Delivery', 'Easy Returns'], icon: '◆', serviceImageQuery: '' },
  ],
  'trades-construction': [
    { name: 'New Builds',           description: 'Full-scale residential and commercial construction from foundations up.',       tags: ['Turnkey', 'Insured'],      icon: '✦', serviceImageQuery: '' },
    { name: 'Renovations',          description: 'Kitchen, bathroom, and structural renovations delivered on time and on budget.', tags: ['NHBRC', 'Guaranteed'],    icon: '◈', serviceImageQuery: '' },
    { name: 'Roofing & Waterproofing', description: 'Expert roof repairs, replacements, and waterproofing across all surfaces.',  tags: ['Warranted', 'Rapid'],      icon: '◉', serviceImageQuery: '' },
    { name: 'Project Management',   description: 'End-to-end oversight keeping your project on schedule and within scope.',       tags: ['Full-Service', 'Transparent'], icon: '◆', serviceImageQuery: '' },
  ],
  'health-wellness': [
    { name: 'GP Consultations',     description: 'Comprehensive primary care consultations with same-day availability.',          tags: ['Same Day', 'Medical Aid'], icon: '✦', serviceImageQuery: '' },
    { name: 'Physiotherapy',        description: 'Evidence-based treatment for injuries, chronic pain, and post-surgical recovery.', tags: ['Accredited', 'HPCSA'],   icon: '◈', serviceImageQuery: '' },
    { name: 'Nutrition Counselling',description: 'Personalised eating plans built around your health goals and lifestyle.',        tags: ['Dietitian', 'Personalised'], icon: '◉', serviceImageQuery: '' },
    { name: 'Mental Wellness',      description: 'Confidential counselling and psychotherapy for individuals and couples.',        tags: ['HPCSA', 'Confidential'],  icon: '◆', serviceImageQuery: '' },
  ],
  'professional': [
    { name: 'Strategic Advisory',   description: 'Business strategy, growth planning, and decision-support for leadership teams.', tags: ['Retainer', 'Senior Only'], icon: '✦', serviceImageQuery: '' },
    { name: 'Legal Services',       description: 'Contracts, compliance, dispute resolution, and corporate structuring.',          tags: ['LSSA', 'Confidential'],   icon: '◈', serviceImageQuery: '' },
    { name: 'Financial Planning',   description: 'Wealth management, tax optimisation, and retirement planning — SARS compliant.',  tags: ['CFP', 'SARS Compliant'],  icon: '◉', serviceImageQuery: '' },
    { name: 'Executive Coaching',   description: 'Structured coaching programmes for founders, MDs, and high-performance teams.',  tags: ['ICF Certified', 'Results'], icon: '◆', serviceImageQuery: '' },
  ],
  'creative': [
    { name: 'Brand Identity',       description: 'Logo, colour system, typography, and brand guidelines that define your voice.',  tags: ['Strategy-Led', 'Full Brand'], icon: '✦', serviceImageQuery: '' },
    { name: 'Art Direction',        description: 'Visual direction for campaigns, editorial shoots, and digital content.',         tags: ['Editorial', 'Campaigns'],  icon: '◈', serviceImageQuery: '' },
    { name: 'Web Design',           description: 'Conversion-focused website design with a distinct visual point of view.',        tags: ['Figma', 'Responsive'],     icon: '◉', serviceImageQuery: '' },
    { name: 'Motion & Video',       description: 'Brand films, social content, and motion graphics for modern audiences.',         tags: ['Reels', 'Brand Films'],    icon: '◆', serviceImageQuery: '' },
  ],
  'fitness-sport': [
    { name: 'Personal Training',    description: 'One-on-one sessions tailored to your fitness level, goals, and schedule.',       tags: ['Personalised', 'Certified'], icon: '✦', serviceImageQuery: '' },
    { name: 'Group Classes',        description: 'High-energy group sessions including HIIT, strength, and functional fitness.',    tags: ['All Levels', 'Daily'],     icon: '◈', serviceImageQuery: '' },
    { name: 'Nutrition Coaching',   description: 'Performance nutrition plans designed to fuel your training and recovery.',        tags: ['Macro Tracking', 'Recomp'], icon: '◉', serviceImageQuery: '' },
    { name: 'Sports Recovery',      description: 'Ice baths, compression therapy, and mobility work for faster recovery.',         tags: ['Cryotherapy', 'Daily'],    icon: '◆', serviceImageQuery: '' },
  ],
  'education': [
    { name: 'Tutoring',             description: 'One-on-one academic support across all subjects for Grades 1–12.',              tags: ['All Grades', 'Results'],   icon: '✦', serviceImageQuery: '' },
    { name: 'Holiday Programmes',   description: 'Intensive revision camps and enrichment workshops during school holidays.',       tags: ['Immersive', 'Fun'],        icon: '◈', serviceImageQuery: '' },
    { name: 'Online Learning',      description: 'Live virtual sessions and recorded lessons accessible from anywhere.',           tags: ['Live & Async', 'Flexible'], icon: '◉', serviceImageQuery: '' },
    { name: 'Exam Preparation',     description: 'Structured NSC and IEB preparation with past paper practice and mock exams.',    tags: ['NSC', 'IEB'],              icon: '◆', serviceImageQuery: '' },
  ],
  'automotive': [
    { name: 'Full Service',         description: 'Comprehensive mechanical service covering all makes and models.',                tags: ['All Makes', 'Warranted'],  icon: '✦', serviceImageQuery: '' },
    { name: 'Panel & Paint',        description: 'Accident repairs, scratch removal, and full respray to factory finish.',         tags: ['Insurance', 'OEM Paint'],  icon: '◈', serviceImageQuery: '' },
    { name: 'Tyres & Alignment',    description: 'Quality tyre fitting and computerised wheel alignment for optimum handling.',    tags: ['All Brands', 'Same Day'],  icon: '◉', serviceImageQuery: '' },
    { name: 'Diagnostics',          description: 'Advanced electronic fault finding and ECU diagnostics for modern vehicles.',     tags: ['OBD-II', 'Fast'],          icon: '◆', serviceImageQuery: '' },
  ],
  'pets': [
    { name: 'Grooming',             description: 'Bath, brush, clip, and style for dogs and cats of all breeds and sizes.',        tags: ['All Breeds', 'Walk-In'],   icon: '✦', serviceImageQuery: '' },
    { name: 'Boarding & Daycare',   description: 'Safe, supervised boarding where your pet is treated like family.',               tags: ['CCTV', 'Daily Updates'],   icon: '◈', serviceImageQuery: '' },
    { name: 'Veterinary Care',      description: 'Preventive check-ups, vaccinations, and wellness consultations on-site.',        tags: ['Registered Vet', 'SAVC'],  icon: '◉', serviceImageQuery: '' },
    { name: 'Training',             description: 'Positive-reinforcement obedience and socialisation programmes for puppies and adults.', tags: ['Certified', 'Results'], icon: '◆', serviceImageQuery: '' },
  ],
  'home-services': [
    { name: 'Deep Cleaning',        description: 'Thorough top-to-bottom cleaning using eco-friendly, pet-safe products.',         tags: ['Eco-Safe', 'Insured'],     icon: '✦', serviceImageQuery: '' },
    { name: 'Garden & Landscaping', description: 'Regular maintenance, lawn care, and landscape design for any size garden.',      tags: ['Weekly', 'Design'],        icon: '◈', serviceImageQuery: '' },
    { name: 'Plumbing & Electrical',description: 'Qualified artisans for leaks, rewiring, faults, and installations.',            tags: ['Licensed', '24h'],         icon: '◉', serviceImageQuery: '' },
    { name: 'Painting & Repairs',   description: 'Interior and exterior painting, plastering, and general handyman services.',     tags: ['CIDB', 'Guaranteed'],      icon: '◆', serviceImageQuery: '' },
  ],
  'property': [
    { name: 'Residential Sales',    description: 'Selling your home with honest valuations, strong marketing, and expert negotiation.', tags: ['Free Valuation', 'FFC'], icon: '✦', serviceImageQuery: '' },
    { name: 'Rentals & Lettings',   description: 'Tenant placement, lease management, and full property management services.',     tags: ['Vetted Tenants', 'NCA'],   icon: '◈', serviceImageQuery: '' },
    { name: 'Commercial Property',  description: 'Office, retail, and industrial leasing and sales across the Western Cape.',      tags: ['Western Cape', 'Offices'], icon: '◉', serviceImageQuery: '' },
    { name: 'Property Investment',  description: 'Buy-to-let strategy, portfolio analysis, and investment property sourcing.',      tags: ['ROI Focus', 'CFP'],        icon: '◆', serviceImageQuery: '' },
  ],
  'events-entertainment': [
    { name: 'Wedding Planning',     description: 'Full-service wedding coordination from concept to final send-off.',              tags: ['Full Coordination', 'Day-Of'], icon: '✦', serviceImageQuery: '' },
    { name: 'Corporate Events',     description: 'Conferences, product launches, team builds, and year-end functions.',            tags: ['AV Included', 'Catering'], icon: '◈', serviceImageQuery: '' },
    { name: 'Décor & Styling',      description: 'Custom table scapes, florals, and installation design for any occasion.',        tags: ['Bespoke', 'Setup & Strike'], icon: '◉', serviceImageQuery: '' },
    { name: 'Entertainment Booking',description: 'Live bands, DJs, MCs, and performers sourced and managed end-to-end.',          tags: ['SAMRO', 'Contracted'],     icon: '◆', serviceImageQuery: '' },
  ],
  'tech-digital': [
    { name: 'Web Development',      description: 'Custom websites and web applications built for speed, scale, and conversion.',   tags: ['React', 'Next.js'],        icon: '✦', serviceImageQuery: '' },
    { name: 'Mobile Apps',          description: 'Native and cross-platform iOS and Android apps from concept to store.',          tags: ['iOS', 'Android'],          icon: '◈', serviceImageQuery: '' },
    { name: 'Digital Marketing',    description: 'SEO, paid media, and content strategy that drives qualified traffic and leads.', tags: ['Google Ads', 'Meta'],      icon: '◉', serviceImageQuery: '' },
    { name: 'IT Support & Cloud',   description: 'Managed IT services, cloud migration, and cybersecurity for SMEs.',             tags: ['AWS', 'Microsoft 365'],    icon: '◆', serviceImageQuery: '' },
  ],
}
const PLACEHOLDER_SERVICES = categoryServices['professional']!

function makeContent(d: MockDef): GeneratedContent {
  const svcDefs = categoryServices[d.cat] || categoryServices['professional']!
  const services = svcDefs.map(s => ({ ...s, serviceImageQuery: d.img }))
  return {
    heroEyebrow: d.eyebrow, heroAccent: 'Award-Winning · Est. 2016',
    tagline: d.tagline, heroSubtitle: d.sub,
    ctaPrimary: d.cta, ctaSecondary: 'Learn More',
    ctaNote: 'Free consultation · No commitment', badge: 'Rated #1 in Cape Town 2024',
    servicesHeading: 'What We Offer', services,
    galleryHeading: 'Our Work',
    aboutHeading: 'Our Story',
    aboutText: 'We started with a simple belief: that great service should be the norm, not the exception. Over the years, we have built a reputation for quality, reliability, and genuine care.\n\nOur team brings decades of combined experience and a passion for excellence to every project.',
    aboutMission: 'Our mission is to exceed expectations, every single time.',
    stats: [
      { value: '12+', label: 'Years Experience', sublabel: 'Industry leading' },
      { value: '500+', label: 'Happy Clients',   sublabel: 'And counting' },
      { value: '98%', label: 'Satisfaction',     sublabel: 'Verified reviews' },
      { value: '24h', label: 'Response Time',    sublabel: 'Always available' },
    ],
    contactHeading: "Let's Work Together",
    contactHours: 'Mon–Fri 8am–6pm · Sat 9am–2pm',
    testimonial: { quote: 'Outstanding service from start to finish. They exceeded every expectation and truly transformed our business.', author: 'Sarah M., Director — Cape Town', rating: 5 },
    testimonials: [
      { quote: "The team's attention to detail and commitment to quality is unmatched.", author: 'James V., CEO', rating: 5 },
      { quote: "Professional, responsive, and genuinely invested in our success.", author: 'Priya N., Business Owner', rating: 5 },
      { quote: "Delivered exactly what they promised, on time and on budget.", author: 'Mark D., Manager', rating: 5 },
    ],
    processSteps: [
      { step: '01', title: 'Discovery',  description: 'We deeply understand your goals, challenges, and vision.' },
      { step: '02', title: 'Strategy',   description: 'We develop a tailored plan to achieve your objectives.' },
      { step: '03', title: 'Execution',  description: 'We deliver with precision, keeping you informed throughout.' },
    ],
    stepsHeading: 'How We Work',
    projectCaptions: ['Premium Project', 'Recent Work', 'Client Success', 'Latest Delivery', 'Featured Case'],
    imageMood: 'professional and aspirational',
    heroImageQuery: d.img, aboutImageQuery: d.img,
    galleryImageQueries: [d.img, d.img, d.img, d.img, d.img],
    features: [
      { name: 'Quality Guaranteed', description: 'We stand behind every project.' },
      { name: 'Expert Team',         description: 'Decades of combined experience.' },
      { name: 'Always Available',    description: '24-hour support.' },
    ],
  }
}

function makeStockImages(imgId: string, category: string = ''): StockImages {
  const base = `https://images.unsplash.com/photo-${imgId}?w=1400&q=80&auto=format&fit=crop`
  const about = `https://images.unsplash.com/photo-${imgId}?w=1000&q=80&auto=format&fit=crop`
  const catPhotos: Record<string, string[]> = {
    'food-hospitality':    ['1414235077428-338989a2e8c0','1504674900247-0877df9cc836','1555396273-367ea4eb4db5','1476224203421-9ac39bcb3327','1482049016688-2d3e1b311543','1547592180-85f173990554','1565299624946-b28f40a0ae38','1567620905732-2d1ec7ab7445','1565958011703-44f9829ba187','1512621776951-a57141f2eefd','1578662996442-48f60103fc96','1441986300917-64674bd600d8'],
    'retail':              ['1441984904996-e0b6ba687e04','1607082348824-0a96f2a4b9da','1472851294608-062f824d29cc','1558618666-fcd25c85cd64','1445205170230-053b83016050','1464863979621-258859e62245','1512436991641-6745cdb1723f','1495474472287-4d71bcdd2085','1460925895917-afdab827c52f','1576013551627-0cc20b96c2a7','1530124566582-a618bc2615dc','1468276311594-df7cb65d8df6'],
    'trades-construction': ['1504307651254-35680f356dfd','1503387762-592deb58ef4e','1581578731548-c64695cc6952','1486325212027-8081e485255e','1566041510639-8d95a2490bfb','1487958449943-2429e8be8625','1518005020951-eccb494ad742','1530124566582-a618bc2615dc','1468276311594-df7cb65d8df6','1576013551627-0cc20b96c2a7','1460925895917-afdab827c52f','1495474472287-4d71bcdd2085'],
    'health-wellness':     ['1571019613454-1cb2f99b2d8b','1506126613408-eca07ce68773','1519823551278-64ac92734fb1','1559757148-5c350d0d3c56','1576091160399-112ba8d25d1d','1518611012118-696072aa579a','1573497620053-ea5300f94f21','1576201836106-db1758fd1c97','1495474472287-4d71bcdd2085','1460925895917-afdab827c52f','1530124566582-a618bc2615dc','1578662996442-48f60103fc96'],
    'professional':        ['1497366216548-37526070297c','1568992687947-868a62a9f521','1519389950473-47ba0277781c','1454165804606-c3d57bc86b40','1486312338219-ce68d2c6f44d','1556761175-4b46a572b786','1542744173-8e7e53415bb0','1497366412874-3415097a27e7','1507003211169-0a1dd7228f2d','1578662996442-48f60103fc96','1441986300917-64674bd600d8','1460925895917-afdab827c52f'],
    'creative':            ['1558618666-fcd25c85cd64','1593642632559-0c6d3fc62b89','1572044162444-ad60f128bdea','1626785774625-ddcddc3445e9','1561214115-f2f134cc4912','1541701494587-cb58502866ab','1618005182384-a83a8bd57fbe','1495474472287-4d71bcdd2085','1578662996442-48f60103fc96','1441986300917-64674bd600d8','1460925895917-afdab827c52f','1576013551627-0cc20b96c2a7'],
    'fitness-sport':       ['1534438327276-14e5300c3a48','1517836357463-d25dfeac3438','1540497077202-7c8a3999166f','1549060279-7e168fcee0c2','1571019614242-c5c5dee9f50b','1581009137042-c552e485697a','1571902943202-507ec2618e8f','1576201836106-db1758fd1c97','1578662996442-48f60103fc96','1530124566582-a618bc2615dc','1468276311594-df7cb65d8df6','1460925895917-afdab827c52f'],
    'education':           ['1503676260728-1c00da094a0b','1522202176988-66273c2fd55f','1427504494785-3a9ca7044f45','1580582932707-520aed937b7b','1543269865-cbf427effbad','1456513080510-7bf3a84b82f8','1541339907198-e08756dedf3f','1430990480609-2bf7c02a6b1a','1604872441539-ef1db9b25f92','1491841573634-28140fc7ced7','1578662996442-48f60103fc96','1495474472287-4d71bcdd2085'],
    'automotive':          ['1492144534655-ae79c964c9d7','1503376780353-7e6692767b70','1502877338535-766e1452684a','1550355291-bbee04a92027','1598300042247-d088f8ab3a91','1555215695-3004980ad54e','1572635148818-ef6fd45eb394','1542362567-b07e54358753','1631295868223-63265b40d9e4','1603386329225-868f9b1ee6c9','1593642702821-c8da6771f0c6','1576013551627-0cc20b96c2a7'],
    'pets':                ['1450778869180-41d0601e046e','1548199973-03cce0bbc87b','1615789591457-74a63395c990','1518717758536-85ae29035b6d','1543466835-00a7907e9de1','1601758124510-52d02ddb7cbd','1560807707-8cc77767d783','1578662996442-48f60103fc96','1495474472287-4d71bcdd2085','1441986300917-64674bd600d8','1460925895917-afdab827c52f','1530124566582-a618bc2615dc'],
    'home-services':       ['1556909114-f6e7ad7d3136','1583847268964-b28dc8f51f92','1581578731548-c64695cc6952','1556761175-4b46a572b786','1585771724684-38269d6639fd','1598928506311-c55ded91a20c','1527515637462-cff94eecc1ac','1536376072261-38c75010e6c9','1530124566582-a618bc2615dc','1468276311594-df7cb65d8df6','1576013551627-0cc20b96c2a7','1460925895917-afdab827c52f'],
    'property':            ['1600585154340-be6161a56a0c','1564013799919-ab600027ffc6','1600607687939-ce8a6c25118c','1568605114967-8130f3a36994','1560518883-ce09059eeffa','1512917774080-9991f1c4c750','1570129477492-45c003edd2be','1568605117036-5fe5e7bab0b7','1556228578-8c89e6adf883','1600047509807-ba8f99d2cdde','1530124566582-a618bc2615dc','1468276311594-df7cb65d8df6'],
    'events-entertainment':['1492684223066-81342ee5ff30','1429962714451-bb934ecdc4ec','1508997449629-303059a039c0','1516450360452-9312f5e86fc7','1470225620780-dba8ba36b745','1511578314322-379afb476865','1519671482749-fd09be7ccebf','1524368535928-5b5e00ddc76b','1530103862676-de8c9debad1d','1527529482837-4698179dc6ce','1576013551627-0cc20b96c2a7','1460925895917-afdab827c52f'],
    'tech-digital':        ['1518770660439-4636190af475','1555066931-4365d14bab8c','1498050108023-c5249f4df085','1504639725590-34d0984388bd','1517694712202-14dd9538aa97','1551288049-bebda4e38f71','1573164713988-8665fc963095','1516110833967-0b5716ca1387','1550751827-4bd374c3f58b','1578662996442-48f60103fc96','1441986300917-64674bd600d8','1460925895917-afdab827c52f'],
  }
  const ids = catPhotos[category] || catPhotos['professional']!
  const cards = ids.map(id => `https://images.unsplash.com/photo-${id}?w=800&q=80&auto=format&fit=crop`)
  return { hero: base, about, cards, avatar: cards[0] }
}

// ─── RUN ALL TEMPLATES ────────────────────────────────────────────────────────

const templateMap: [string, (data: TemplateData) => string][] = [
  ['visual',         buildVisualTemplate],
  ['service',        buildServiceTemplate],
  ['portfolio',      buildPortfolioTemplate],
  ['foodHospitality',buildFoodHospitalityTemplate],
  ['retail',         buildRetailTemplate],
  ['pets',           buildPetsTemplate],
  ['healthWellness', buildHealthWellnessTemplate],
  ['fitness',        buildFitnessTemplate],
  ['education',      buildEducationTemplate],
  ['professional',   buildProfessionalTemplate],
  ['techDigital',    buildTechDigitalTemplate],
  ['trades',         buildTradesTemplate],
  ['homeServices',   buildHomeServicesTemplate],
  ['automotive',     buildAutomotiveTemplate],
  ['property',       buildPropertyTemplate],
  ['creative',       buildCreativeTemplate],
  ['events',         buildEventsTemplate],
]

const OUTDIR = '/Users/hugodrummond/agency-backend/templates'
let ok = 0, fail = 0

for (const [key, fn] of templateMap) {
  try {
    const d = mockDataByCategory[key]!
    const data: TemplateData = {
      content: makeContent(d),
      businessName: d.name,
      businessCategory: d.cat,
      primaryColor: d.color,
      secondaryColor: '#888888',
      pages: ['Services', 'Gallery', 'About', 'Contact'],
      images: [makeStockImages(d.img, d.cat).hero],
      stockImages: makeStockImages(d.img, d.cat),
      variant: 'visual',
      locationInfo: getLocationInfo('South Africa'),
    }
    const html = fn(data)
    writeFileSync(`${OUTDIR}/preview-${key}.html`, html)
    console.log(`✓ preview-${key}.html`)
    ok++
  } catch(e: any) {
